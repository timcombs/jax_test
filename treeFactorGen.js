...
var TreeGenerator = function (canvas, opts) {
  ...
  function branch(x, y, dx, dy, w, growthRate, lifetime, branchColor) {
    const wFactor = Math.random() * 0.75 + 0.25;
    canvas.ctx.lineWidth = 1 * w * wFactor - lifetime * tg.settings.loss;
    ...
    if (w < 6 && y > canvas.HEIGHT - Math.random() * (0.3 * canvas.HEIGHT))
      w = w * 0.8;
    ...
    if (
      ...
      setTimeout(function () {
        ...
        branch(
          ...
          (w - lifetime * tg.settings.loss) * tg.settings.branchLoss,
  ...
  function fade() {
    ...
    canvas.ctx.fillStyle =
      'rgba(' +
      tg.settings.bgColor[0] +
      ',' +
      tg.settings.bgColor[1] +
      ',' +
      tg.settings.bgColor[2] +
      ',' +
      tg.settings.fadeAmount +
      ')';
   ...
   */
  function newColor() {
    if (!tg.settings.colorful) return '#ff3d51'; // TODO: should be treeColor

    const newClr = Math.floor(Math.random() * 3);
    if (newClr === 0) return '#3DFFE4';
    if (newClr === 1) return '#C7FF3D';
    if (newClr === 2) return '#ff3d51';
  }
